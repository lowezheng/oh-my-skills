...
| Agent | 用途 | Session | 原因 |
|-------|------|---------|------|
| **Metis** | 意图识别、Gap分析 | Current | 需当前上下文 |
| **Momus** | 计划审查 | Current | 需当前上下文 |
| **Explore** | 代码库探索 | Sub | 独立探索 |
| **Librarian** | 外部研究、文档发现 | Sub | 独立研究 |
| **Oracle** | 高层推理、架构决策 | Sub | 高成本推理 |
| **General** | 通用分析（低成本） | Sub | 低成本分析 |
| **Multimodal-Looker** | 媒体分析（PDF/图片） | Sub | 媒体处理 |
...
- 禁止修改 `.plans/` 之外的文件
- 禁止在生成计划前调用 Momus
- 禁止对 Metis/Momus 使用 `task` 工具（必须当前 Session）
- 禁止超过 2 次迭代后不询问用户
